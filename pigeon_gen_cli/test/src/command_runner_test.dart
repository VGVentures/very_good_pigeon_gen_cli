...
void main() {
  group('PigeonGenCliCommandRunner', () {
    ...
    setUp(() {
      ...
      commandRunner = PigeonGenCliCommandRunner(
        logger: logger,
    ...
    test(
      ...
      () {
        final commandRunner = PigeonGenCliCommandRunner();
    ...
    group('--verbose', () {
      ...
      test('enables verbose logging for sub commands', () async {
        ...
        final absoluteUri = Uri.file(cwd.path).toString();
        final json =
            '''
{
  "packages": [
    { "name": "absolute_pkg", "rootUri": "$absoluteUri" },
    { "name": "relative_pkg", "rootUri": "../../relative_root" }
  ]
}
''';
        ...
        verify(() => logger.detail('  Top level options:')).called(1);
        verify(() => logger.detail('  - verbose: true')).called(1);
        verify(() => logger.detail('  Command: gen')).called(1);
        verify(() => logger.detail('    Command options:')).called(1);
        verify(() => logger.detail('    - input: ${inputFile.path}')).called(1);
      });
    });
  });
}
